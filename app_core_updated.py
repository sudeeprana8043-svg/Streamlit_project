...
def load_all_models(progress=None):
    ...
    log("Checking new multitask model files...")
    ensure_model_files()

    log("Loading multitask metadata and label encoders...")
    ...
    log(
        f"Loading frozen multitask backbone: {backbone_id}"
    )
    ...
    log("Loading trained multitask temporal model...")

    multitask_model = MultiTaskTemporalModel(
        metadata
    )

    state_dict = load_state_dict_from_pkl(
        MULTITASK_MODEL_PATH
    )
    ...
    log("Loading LoRA summarization model...")
    ...
    try:
        ...
        log(
            "LoRA summarization checkpoint could not be loaded; "
            f"using base model. Error: {error}"
        )
    ...
    log("All new-architecture models loaded.")
...
def predict_multiclass(
    ...
    """
    Uses the exact trained architecture:
      frame features -> Temporal Adapter
      training-style generated summary -> summary feature
      -> Summary Projector -> Fusion -> 4 heads

    The UI/LoRA summary argument is intentionally NOT used as the
    classifier summary because training generated summaries with the
    frozen multitask backbone and a specific factual prompt.
    """
    ...
    # Keep the original Streamlit return order:
    # people, weapon, location, category, actions
    return (
        str(predictions["people"]),
        str(predictions["weapon"]),
        str(predictions["location"]),
        str(predictions["activity"]),
        None,
    )
