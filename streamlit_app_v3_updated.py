# ==========================================================
# STREAMLIT VIDEO QA SYSTEM - NEW MULTITASK ARCHITECTURE
# ==========================================================
...
# ==========================================================
# STAGE 2 - NEW TEMPORAL + SUMMARY-FUSION MULTITASK MODEL
# ==========================================================
...
def ensure_attrs(state_key, models):
    ...
    with st.spinner(
        "Running Temporal Adapter + Summary Fusion..."
    ):
        (
            people,
            weapon,
...
def render_results(
    ...
    with col1:
        ...
        binary_parts = ctx.get(
            "binary_parts",
            {},
        )

        if binary_parts:
            simple_score = binary_parts.get(
                "simple",
                0.0,
            )

            lora_score = binary_parts.get(
                "lora",
                0.0,
            )

            st.caption(
                f"SimpleAdapter: {simple_score:.4f} | "
                f"LoRA: {lora_score:.4f}"
            )
    ...
    st.caption(
        "Activity, weapon, people and location are predicted "
        "on demand using the trained Temporal Adapter + "
...
2. **Ask** → Activity, people, weapon and location using the
   trained Temporal Adapter + Summary Fusion model.
"""
)
